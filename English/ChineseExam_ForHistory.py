...
def LoadVocabulary(user_name):
    user_cvs_file = user_name+'_all_exam_Vocabulary_Chinese.csv'
    if os.path.isfile(user_cvs_file) == True:
        # 假設檔案已存在則讀取檔案, 並確認是否有更新
        org_rows = []
        with open('all_exam_Vocabulary.csv', 'r', encoding='utf-8-sig', newline='') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                dict1 = {'VOCABULARY':row['ENGLISH'], 'TYPE':'no', 'CHINESE':row['CHINESE'], 
...
def UpdateVocabulary(user_name):
    user_cvs_file = user_name+'_all_exam_Vocabulary.csv'
...
def UpdateChoiseItems(self):          
    ...
    self.tempbuf[3] = random.randrange(0, self.nMaxSize-1)
    self.tempbuf[4] = random.randrange(0, self.nMaxSize-1)
    self.tempbuf[5] = self.examIdx

    rs = random.getstate()
    r0 = [random.randint(0, 100) for _ in range(10)]
    random.setstate(rs)
    self.answerList = random.sample(self.tempbuf, 6)
    ...
    ans01 = g_rows[self.answerList[4]]
    self.select_05.set(ans01['CHINESE'])

    ans01 = g_rows[self.answerList[5]]
    self.select_06.set(ans01['CHINESE'])
...
class winMain(tk.Frame):
    def __init__(self, root):
        ...
        self.answerList = [0, 0, 0, 0, 0 , 0]
        self.tempbuf = [0, 1, 2, 3, 4, 5]
        ...
        self.select_05 = tk.StringVar()
        self.select_06 = tk.StringVar()
        ...
        if self.row['WRONG_TIMES'] == 0:
            self.exam.set('英文: '+ self.row['VOCABULARY']  )
        else:
            self.exam.set('英文: '+ self.row['VOCABULARY'] + '  - 錯誤率 ' + str(self.row['WRONG_TIMES'] ) + ' / '+ str(self.row['EXAM_TIMES'] ))
        ...
        self.btn_comfirm_01.place(x=20, y=110)
        ...
        self.btn_comfirm_02.place(x=210, y=110)
        ...
        self.btn_comfirm_03.place(x=20, y=160)
        ...
        self.btn_comfirm_04.place(x=210, y=160)     

        self.btn_comfirm_05 = tk.Button(root, textvariable=self.select_05,  font=('Arial', 12), width = 18,
        command=partial(self.OnCheck, self.answerList[4]))
        self.btn_comfirm_05.place(x=20, y=210)

        self.btn_comfirm_06 = tk.Button(root, textvariable=self.select_06,  font=('Arial', 12), width = 18,
        command=partial(self.OnCheck, self.answerList[5]))
        self.btn_comfirm_06.place(x=210, y=210)    
    ...
    def OnCheck(self, num):
        ...
        if num == self.examIdx:
           # 假設之前有錯誤, 則存入 array 裡
            ...
            self.btn_comfirm_05.config(command=partial(self.OnCheck, self.answerList[4]))
            self.btn_comfirm_06.config(command=partial(self.OnCheck, self.answerList[5]))
...
if __name__ == "__main__":
    ...
    def OnSign(event):
        ...
        g_profile['EXAMPLE_CK'] = 0 #int(ck_eaxmple.get())
